...
func (u UTXOSet) FindSpendableOutputs(pubKeyHash []byte, amount int) (int, map[string][]int) {
	...
	err := db.View(func(txn *badger.Txn) error {
		...
		for iter.Seek(utxoPrefix); iter.ValidForPrefix(utxoPrefix); iter.Next() {
			...
			// this might be enhanced by resuing slice
...
func (u *UTXOSet) Update(block *Block) error {
	...
	err := db.Update(func(txn *badger.Txn) error {
		for _, tx := range block.Transactions {
			if tx.IsCoinbase() == false {
				for _, in := range tx.Inputs {
					...
					if len(updatedOuts.Outputs) == 0 {
						...
						if err := txn.Set(inID, serialized); err != nil {
							log.Panic(err)
						}
					}
				}
			...
			if err := txn.Set(txID, serialized); err != nil {
				log.Panic(err)
			}
		}
...
func (u *UTXOSet) DeleteByPrefix(prefix []byte) {
	deleteKeys := func(keysForDelete [][]byte) error {
		if err := u.Blockchain.DB.DB.Update(func(txn *badger.Txn) error {
	...
	collectSize := 100000
	u.Blockchain.DB.DB.View(func(txn *badger.Txn) error {
		opts := badger.DefaultIteratorOptions
